...
export class TokenDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getTokenDetails(this.tokenListItem.maPolicyId, this.tokenListItem.maName).subscribe(details => {
      this.tokenDataWithMetadata = this.tokenEnhancerService.enhanceToken(details);
  ...
  buildShareUrl() {
    return window.location.origin + this.location.prepareExternalUrl('latest?q=' + this.tokenDataWithMetadata?.fingerprint);
  }
  ...
  copyToClipboard(value: string) {
    ...
    this.snackBar.open('Copied to clipboard: ' + value, undefined, { duration: 2000 });
  }

  copyMetadataToClipboard() {
    this.clipboard.copy('"' + this.tokenDataWithMetadata!.maName + '": ' + JSON.stringify(JSON.parse(this.tokenDataWithMetadata!.metadata!), null, 3) + ',');
  }
