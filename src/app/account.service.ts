...
export class AccountService {
  readonly account: ReplaySubject<AccountPrivate> =
    new ReplaySubject<AccountPrivate>(1);
  readonly funds: ReplaySubject<number> = new ReplaySubject<number>(1);
  readonly fundingAddresses: ReplaySubject<string[]> = new ReplaySubject<
    string[]
  >(1);
  readonly policies: ReplaySubject<PolicyPrivate[]> = new ReplaySubject<
    PolicyPrivate[]
  >(1);
    ...
    this.account
      ...
      .subscribe((utxos) => {
        this.fundingAddresses.next([
          ...new Set(utxos.map((utxo) => utxo.sourceAddress)),
        ]);
        this.funds.next(
          utxos
