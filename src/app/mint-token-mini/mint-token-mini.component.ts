...
export class MintTokenMiniComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.metaData =
      JSON.parse(this.mintOrderSubmission.metaData!)?.['721']?.[
        this.mintOrderSubmission.policyId
      ]?.[this.token.assetName!] || {};
  ...
  get tableData(): TableRow[] {
    let data: TableRow[] = [{ name: 'Amount', value: this.token.amount }];
